# == AUTO MAPFAST BUILD: vectorized with CasADi Function.map where possible ==
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class CasadiModel:
    ...
    # Vectorized helpers (numpy arrays in, numpy arrays out), using map('serial')
    def f(self, x_vec: np.ndarray, i_val: float, theta_vec: np.ndarray) -> np.ndarray:
        xv = np.asarray(x_vec, dtype=float).ravel()
        N = int(xv.size)
        if N == 0:
            return np.zeros(0, dtype=float)
        X = ca.DM(xv).T
        I = ca.DM(np.full((1, N), float(i_val)))
        TH = ca.DM(np.asarray(theta_vec, dtype=float).reshape(-1, 1))
        TH = ca.repmat(TH, 1, N)
        fmap = self.f_fun.map(N, 'serial')
        out = fmap(X, I, TH)
    ...
    def dfdx(self, x_vec: np.ndarray, i_val: float, theta_vec: np.ndarray) -> np.ndarray:
        ...
        TH = ca.DM(np.asarray(theta_vec, dtype=float).reshape(-1, 1))
        TH = ca.repmat(TH, 1, N)
        fmap = self.dfdx_fun.map(N, 'serial')
        out = fmap(X, I, TH)
        return np.asarray(out).ravel()
    ...
    def d2fdx2(self, x_vec: np.ndarray, i_val: float, theta_vec: np.ndarray) -> np.ndarray:
        ...
        TH = ca.DM(np.asarray(theta_vec, dtype=float).reshape(-1, 1))
        TH = ca.repmat(TH, 1, N)
        fmap = self.d2fdx2_fun.map(N, 'serial')
        out = fmap(X, I, TH)
        return np.asarray(out).ravel()
...
def main() -> None:
    ...
    # Build config
    cfg = SolverConfig(eval_mode='map-serial', loss='huber', 
